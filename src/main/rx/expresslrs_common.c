...
static elrs_freq_domain_e domain;

elrs_mod_settings_t air_rate_config[2][ELRS_RATE_MAX] = {
    { //0
    ...
    { //1
    //    {0, RATE_250HZ, SX1280_LORA_BW_0800, SX1280_LORA_SF6, SX1280_LORA_CR_LI_4_7, 4000, TLM_RATIO_1_64, 4, 14, -108},
    //    {1, RATE_150HZ, SX1280_LORA_BW_0800, SX1280_LORA_SF7, SX1280_LORA_CR_LI_4_7, 6666, TLM_RATIO_1_32, 4, 12, -112},
    //    {2, RATE_50HZ, SX1280_LORA_BW_0800, SX1280_LORA_SF9, SX1280_LORA_CR_LI_4_6, 20000, TLM_RATIO_NO_TLM, 4, 12, -117},
    //    {3, RATE_25HZ, SX1280_LORA_BW_0800, SX1280_LORA_SF10, SX1280_LORA_CR_LI_4_6, 40000, TLM_RATIO_NO_TLM, 4, 12, -120}
...
{
    domain = dom;
    return ((domain == ISM2400) ? sx1280Init(resetPin, busyPin) : sx127xInit(resetPin));
}
...
{
    ((domain == ISM2400) ? sx1280Config(bw, sf, cr, freq, preambleLen, iqInverted) : sx127xConfig(bw, sf, cr, freq, preambleLen, iqInverted));
}

void elrsRxStartReceiving(void)
{
    ((domain == ISM2400) ? sx1280StartReceiving() : sx127xStartReceiving());
}

bool elrsRxISR(timeUs_t *timeStamp)
{
    return ((domain == ISM2400) ? sx1280ISR(timeStamp) : sx127xISR(timeStamp));
}

void elrsRxTransmitData(const uint8_t *data)
{
    ((domain == ISM2400) ? sx1280TransmitData(data, ELRS_RX_TX_BUFF_SIZE) : sx127xTransmitData(data, ELRS_RX_TX_BUFF_SIZE));
}

void elrsRxReceiveData(uint8_t *data)
{
    ((domain == ISM2400) ? sx1280ReceiveData(data, ELRS_RX_TX_BUFF_SIZE) : sx127xReceiveData(data, ELRS_RX_TX_BUFF_SIZE));
}

void elrsRxGetRFlinkInfo(int8_t *rssi, int8_t *snr)
{
    if (domain == ISM2400) {
        sx1280GetLastPacketStats(rssi, snr);
    } else {
...
{
    ((domain == ISM2400) ? sx1280SetFrequencyReg(freq) : sx127xSetFrequencyReg(freq));
}

bool elrsRxGetFrequencyErrorbool(void)
{
    return ((domain == ISM2400) ? sx1280GetFrequencyErrorbool() : sx127xGetFrequencyErrorbool());
}

void elrsRxSetPPMoffsetReg(const int32_t offset, const uint32_t freq)
{
    if (domain != ISM2400) {
        sx127xSetPPMoffsetReg(offset, freq);
    } 
}
